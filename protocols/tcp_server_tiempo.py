...
def handle_client(conn, addr):
    ...
    params = {
        "amplitud": 30.0,
        "hz": 100.0,
        "segundos": 9999999,  # Transmisión infinita por defecto
        "stop": False
    }

    lock = threading.Lock()

    # Obtener valores iniciales del cliente
    try:
        buffer = ""
    ...
    # Hilo para recibir cambios de parámetros
    ...
    try:
        while not params["stop"]:
            with lock:
                ...
                duracion = params["segundos"]

            current_time = time.time() - start_time
            if current_time > duracion:
                break
